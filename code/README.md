...
| `transformer_block.py` | stdlib only | **Authoritative for page 07.** One forward pass of a tiny pre-norm transformer block, emitting `assets/data/transformer.{json,js}`. The MLP trace has no transformer in it, so page 07 gets its own generator under the same rules. |
...
FlashAttention assumption stated. **Site page 08 (`site/08-scaling.html`) must
...
The gap between those two commands is the entire subject of page 08.
...
same published architectures and HBM capacities page 08 puts in its
