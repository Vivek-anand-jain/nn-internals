...
| `transformer_block.py` | stdlib only | **Authoritative for page 13.** One forward pass of a tiny pre-norm transformer block, emitting `assets/data/transformer.{json,js}`. The MLP trace has no transformer in it, so page 13 gets its own generator under the same rules. |
...
FlashAttention assumption stated. **Site page 15 (`site/15-scaling.html`) must
...
The gap between those two commands is the entire subject of page 15.
...
same published architectures and HBM capacities page 15 puts in its
