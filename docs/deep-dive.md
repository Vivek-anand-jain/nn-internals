...
Let `X` be `(B, k)` and `A` be `(k, n)`, so `Y = XA` is `(B, n)`. Cut `A` into
`N` column blocks:
...
**Proof.** `Y[b][j] = sum_{t=0}^{k-1} X[b][t] · A[t][j]`. The index `j` ranges
over output columns, and every output column `j` belongs to exactly one block
`i = floor(jN/n)`. The sum over `t` never crosses a column boundary — it runs
...
X = [ X_1 | X_2 | ... | X_N ]        each X_i is (b, n/N)
```
...
**Proof.** `Y[b][j] = sum_{t=0}^{n-1} X[b][t] · B[t][j]`. Split the sum over
`t` at the block boundaries:

```
Y[b][j] = sum_{i} ( sum_{t in block i} X[b][t] · B[t][j] )
        = sum_{i} (X_i B_i)[b][j]
```
...
- Rank `i` produces `X_i B_i`, which is **full-shape** `(B, m)` but is only a
...
concatenating the `f`s, because `f` maps entry `(b, j)` to a function of entry
`(b, j)` alone and never mixes columns. Rank `i` can therefore compute
