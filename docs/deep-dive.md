...
| Part | Sections | Pages |
|---|---|---|
| I — the mechanics, on the 13-parameter MLP | 1-8 | `site/01-memory.html`, `site/02-forward.html`, `site/03-loss.html`, `site/04-backward.html`, `site/05-optimizer.html`, `site/07-loop.html` |
| II — a real transformer | 9-11 | `site/09-transformer-forward.html`, `site/10-transformer-backward.html`, `site/13-transformer-cost.html`, `site/14-flash-attention.html`, `site/15-scaling.html` |
...
`site/07-loop.html` animates this.
...
`site/07-loop.html` shows the four classes rising and falling across one
iteration.
...
central finite differences. `site/09-transformer-forward.html` animates this
section; `site/10-transformer-backward.html` animates section 10.
...
- **`torch.nn.Linear` stores the transpose.** Its `weight` attribute has shape
  ...
  `site/19-tensor-parallel.html` covers the trap; `site/22-transformer-partitioned.html`
...
loss is provably flat. `site/10-transformer-backward.html` re-runs the
...
Page 10 caught it; the fix is in the data, not in the prose.)
...
`TF2.dOut`/`TF2.dx_in`, exactly as `site/10-transformer-backward.html`
computes it live):
...
and `site/14-flash-attention.html` steps through the tiles.
...
`site/16-collectives.html` animates each primitive on four ranks.
...
`site/17-data-parallel.html` runs the four-rank version step by step.
...
`site/18-zero-fsdp.html` steps through the three stages on four ranks.
...
`site/19-tensor-parallel.html` splits the toy's hidden layer four ways and
...
`site/20-sequence-parallel.html` sits directly after the tensor-parallel page
...
`site/21-pipeline-parallel.html` animates the grid below filling and draining.
...
`site/23-3d-parallelism.html` lets you factor a world size and see the
...
**Step 0 — the problem.** (`site/15-scaling.html` runs this budget
...
- **The peak is not the sum.** Peak memory is the max over time of live
  ...
  a table — which is what `site/07-loop.html` is for.
